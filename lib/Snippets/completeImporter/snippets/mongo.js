...
const getSnippetForFunction = function(word, {file, preText, prefix, fileContent, bufferPosition}) {
  ...
  if (!file.path.includes('server/imports')) return
  return {
    snippet: `${word}($1)`,
    displayText: `${collectionName === 'users' ? 'Meteor.users' : collectionName}.${word}`,
    type: 'snippet',
    description: `Adds ${word} and imports ${
      collectionName === 'users' ? 'Meteor' : `the ${collectionName} collection`
    }`,
    ...
    onDidInsertSuggestion: function({fileContent, editor}) {
      ...
      if (collectionName === 'users') {
        ...
        const collectionPath = file.path.replace(
          /imports\/.*/g,
          `imports/collections/${collectionName}/index.js`
        )
        if (!collectionPath.includes('imports/collections')) return
...
export default function({file, preText, prefix, fileContent, bufferPosition}) {
  ...
  const snippets = ['find', 'findOne', 'insert', 'remove', 'update']
    .map(word => {
